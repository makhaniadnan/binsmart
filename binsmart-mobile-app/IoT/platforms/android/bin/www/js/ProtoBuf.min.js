	...
	function u(l) {
		...
		g.DotProto = function(e, d) {
			...
			n._readString = function() {
				...
				if (null !== ( f = d.STRING.exec(this.source)))
					return f = "undefined" !== typeof f[1] ? f[1] : f[2], this.index = d.STRING.lastIndex, this.stack.push(this.stringEndsWith), f;
			...
			b._parseId = function(f, a) {
				...
				if (d.NUMBER_DEC.test(f))
					c = parseInt(f);
				else if (d.NUMBER_HEX.test(f))
					c = parseInt(f.substring(2), 16);
				else if (d.NUMBER_OCT.test(f))
			...
			b._parseOption = function(f, a) {
				...
				if (a === d.STRINGOPEN || a === d.STRINGOPEN_SQ)
					c = this._parseString();
				else if (this.tn.next(), d.NUMBER.test(a))
					c = this._parseNumber(a, !0);
				else if (d.BOOL.test(a))
					c = "true" === a;
				else if (d.TYPEREF.test(a))
					c = a;
				else
			...
			b._parseMessage = function(f, a, c) {
				...
				b.oneofs = {};
				c = this.tn.next();
				c === d.OPTOPEN && a && (this._parseFieldOptions(b, a, c),
				c = this.tn.next());
				if (c !== d.OPEN)
			...
			b._parseMessageField = function(b, a) {
				...
				if ("group" === a) {
					...
					a = this.tn.next();
					a === d.OPTOPEN && (this._parseFieldOptions(b, c, a),
					a = this.tn.next());
					if (a !== d.END)
			...
			b._parseMessageOneOf = function(b, a) {
				...
				for (; this.tn.peek() !== d.CLOSE; )
					e = this._parseMessageField(b, "optional"), e.oneof =
					c, g.push(e.id);
				this.tn.next();
				b.oneofs[c] = g
			};
			...
			b._parseFieldOption = function(b, a, c) {
				...
				if (c === d.STRINGOPEN || c === d.STRINGOPEN_SQ)
					b = this._parseString();
				else if (d.NUMBER.test(c, !0))
					b = this._parseNumber(this.tn.next(), !0);
				else if (d.BOOL.test(c))
					b = "true" === this.tn.next().toLowerCase();
				else if (d.TYPEREF.test(c))
					b = this.tn.next();
				else
			...
			b._parseEnumValue = function(b, a) {
				...
				b.values.push(c);
				a = this.tn.next();
				a === d.OPTOPEN && (this._parseFieldOptions(b, {
					options : {}
				}, a),
				a = this.tn.next());
				if (a !== d.END)
		...
		g.Reflect = function(e) {
			...
			b.getChild = function(a) {
				for (var b = "number" === typeof a ? "id" : "name",
				    c = 0,
				    e = this.children.length; c < e; ++c)
			...
			c.build = function(b) {
				...
				b = function(a, b) {
					function c(a, b) {
						var e = {},
						    m;
						for (m in a)a.hasOwnProperty(m) && (null === a[m] || "object" !== typeof a[m] ? e[m] = a[m] : a[m] instanceof l ? b && (e[m] = a[m].toBase64()) : e[m] = c(a[m], b));
						return e
					}

					var e = b.getChildren(a.Reflect.Message.Field),
						...
						if (0 < arguments.length)
							if (1 !== arguments.length || "object" !== typeof b || "function" === typeof b.encode || a.Util.isArray(b) || b instanceof l || b instanceof ArrayBuffer || a.Long && b instanceof a.Long)
								for ( d = 0,
					...
					d.add = function(c, e, d) {
						...
						if (!d) {
							...
							if (!( m instanceof a.Reflect.Message.Field))
								throw Error(this + "#" + c + " is not a field: " + m.toString(!0));
							if (!m.repeated)
								throw Error(this + "#" + c + " is not a repeated field");
						}
						null === this[m.name] && (this[m.name] = []);
						this[m.name].push( d ? e : m.verifyValue(e, !0))
					};
					d.$add = d.add;
					d.set = function(c, e, m) {
						if (c && "object" === typeof c) {
							for (var d in c)c.hasOwnProperty(d) && this.$set(d, c[d], m);
							return this
						}
						d = b._fieldsByName[c];
						if (m)
							...
							this[d.name] = e = d.verifyValue(e)
						}
						d.oneof && (null !== e ? (null !== this[d.oneof.name] && (this[this[d.oneof.name]] = null), this[d.oneof.name] = d.name) : d.oneof.name === c && (this[d.oneof.name] = null));
						return this
					};
					d.$set = d.set;
					d.get = function(c, d) {
					...
					for (var q = 0; q < e.length; q++) {
						...
						g instanceof a.Reflect.Message.ExtensionField || b.builder.options.populateAccessors && function(a) {
							var c = a.originalName.replace(/(_[a-zA-Z])/g, function(a) {
							    ...
							    e = a.originalName.replace(/([A-Z])/g, function(a) {
								return "_" + a
							}),
							    m = function(b, c) {
							...
							null === b.getChild("set" + c) && (d["set" + c] = m);
							null === b.getChild("set_" + e) && (d["set_" + e] = m);
							null === b.getChild("get" + c) && (d["get" + c] = f);
							null === b.getChild("get_" + e) && (d["get_" + e] = f)
						}(g)
					}
					...
					f.decode = function(a, c) {
						"string" === typeof a && ( a = l.wrap(a, c ? c : "base64"));
						a = a instanceof l ? a : l.wrap(a);
					...
					f.decodeDelimited = function(a, c) {
						...
						var e = a.offset,
						    d = a.readVarint32();
						if (a.remaining() < d)
							return a.offset = e, null;
						try {
							var m = b.decode(a.slice(a.offset, a.offset + d).LE());
							a.offset += d;
							return m
						} catch(f) {
							throw a.offset += d, f;
				...
				for (var c = 0,
					...
					if ( d = this.children[c], d instanceof p)
						...
						b[d.name] = d.build();
					else if ( d instanceof a.Field)
						d.build(), this._fields.push(d), this._fieldsById[d.id] =
						d, this._fieldsByName[d.name] =
						d;
					else if (!( d instanceof a.OneOf || d instanceof h))
			...
			c.encode = function(a, b, c) {
				for (var e = null,
				    d,
				    f = 0,
				    g = this._fields.length,
				    h; f < g; ++f)
					d = this._fields[f],
					h = a[d.name], d.required && null === h ? null === e && ( e = d) : d.encode( c ? h : d.verifyValue(h), b);
				if (null !== e)
					throw a = Error("Missing at least one required field for " + this.toString(!0) + ": " + e), a.encoded =
					b, a;
				return b
			};
			...
			c.decode = function(a, b, c) {
				...
				for (var f = a.offset,
				    ...
				    h,
				    p,
				    k; a.offset < f + b || -1 === b && 0 < a.remaining(); ) {
					h = a.readVarint32();
					p = h & 7;
					k = h >> 3;
				...
				for ( b = this._fields.length; a < b; ++a)
					if ( h = this._fields[a], null === g[h.name]) {
						if (h.required)
							throw a = Error("Missing at least one required field for " + this.toString(!0) + ": " + h.name), a.decoded =
							g, a;
			...
			k.Message = a;
			var r = function(b, c, d, e, f, g, h, p) {
				n.call(this, b, c, f);
			...
			c.verifyValue = function(a, b) {
				...
				case e.TYPES.bytes:
					return l.isByteBuffer(a) ? a : l.wrap(a, "base64");
				case e.TYPES["enum"]:
				...
				try {
					if (this.repeated) {
						...
						if (this.options.packed && 0 <= e.PACKABLE_WIRE_TYPES.indexOf(this.type.wireType)) {
							...
							var f = b.offset - d,
							    g = l.calculateVarint32(f);
							if (1 < g) {
								var h = b.slice(d, b.offset),
								    d = d + (g - 1);
								b.offset = d;
								b.append(h)
							}
				...
				try {
					if (this.repeated) {
						var c,
						    d;
						if (this.options.packed && 0 <= e.PACKABLE_WIRE_TYPES.indexOf(this.type.wireType)) {
							b += l.calculateVarint32(this.id << 3 | e.WIRE_TYPES.LDELIM);
							for ( c = d = 0; c < a.length; c++)
								d += this.calculateValue(a[c]);
							b += l.calculateVarint32(d);
							b += d
						} else
							for ( c = 0; c < a.length; c++)
								b += l.calculateVarint32(this.id << 3 | this.type.wireType), b += this.calculateValue(a[c])
					} else
						b += l.calculateVarint32(this.id << 3 | this.type.wireType), b += this.calculateValue(a)
				} catch(f) {
			...
			c.calculateValue = function(a) {
				...
				switch(this.type) {
				case e.TYPES.int32:
					return 0 > a ? l.calculateVarint64(a) : l.calculateVarint32(a);
				case e.TYPES.uint32:
					return l.calculateVarint32(a);
				case e.TYPES.sint32:
					return l.calculateVarint32(l.zigZagEncode32(a));
				case e.TYPES.fixed32:
				case e.TYPES.sfixed32:
				case e.TYPES["float"]:
					return 4;
				case e.TYPES.int64:
				case e.TYPES.uint64:
					return l.calculateVarint64(a);
				case e.TYPES.sint64:
					return l.calculateVarint64(l.zigZagEncode64(a));
				case e.TYPES.fixed64:
				case e.TYPES.sfixed64:
					return 8;
				case e.TYPES.bool:
				...
				case e.TYPES["double"]:
					return 8;
				case e.TYPES.string:
					return a = l.calculateUTF8Bytes(a), l.calculateVarint32(a) + a;
				case e.TYPES.bytes:
					if (0 > a.remaining())
						throw Error("Illegal value for " + this.toString(!0) + ": " + a.remaining() + " bytes remaining");
					return l.calculateVarint32(a.remaining()) + a.remaining();
				case e.TYPES.message:
					return a = this.resolvedType.calculate(a), l.calculateVarint32(a) + a;
				case e.TYPES.group:
					return a = this.resolvedType.calculate(a), a + l.calculateVarint32(this.id << 3 | e.WIRE_TYPES.ENDGROUP)
				}
				throw Error("[INTERNAL] Illegal value to encode in " + this.toString(!0) + ": " + a + " (unknown type)");
			};
			c.decode = function(a, b, c) {
			...
			(c.prototype = Object.create(f.prototype)).build = function(a) {
				return this.clazz && !a ? this.clazz : this.clazz = function(a, b) {
					...
					for (var e = b.getChildren(a.Reflect.Service.RPCMethod),
					    f = 0; f < e.length; f++)
						(function(a) {
			...
			k.isValidMessageField = function(b) {
				...
				if ("undefined" !== typeof b.options) {
					if ("object" !== typeof b.options)
						return !1;
					for (var e = Object.keys(b.options),
					    a = 0,
					    c; a < e.length; a++)
					...
					if (e.Util.isArray(b))
						for (; 0 < b.length; ) {
							...
							if (k.isValidMessage(a)) {
								var c = new g.Message(this, this.ptr, a.name, a.options, a.isGroup),
								    n = {};
								if (a.oneofs)
								...
								if (a.fields && 0 < a.fields.length)
									for ( h = 0,
									l = a.fields.length; h < l; ++h) {
										p = a.fields[h];
										if (null !== c.getChild(p.id))
											throw Error("Duplicate field id in message " + c.name + ": " + p.id);
										if (p.options)
											for (var m = Object.keys(p.options),
											    q = 0,
											    s = m.length; q < s; ++q) {
												if ("string" !== typeof m[q])
													throw Error("Illegal field option name in message " + c.name + "#" + p.name + ": " + m[q]);
												if ("string" !== typeof p.options[m[q]] && "number" !== typeof p.options[m[q]] && "boolean" !== typeof p.options[m[q]])
													throw Error("Illegal field option value in message " + c.name + "#" + p.name + "#" + m[q] + ": " + p.options[m[q]]);
											}
										m = null;
										if ("string" === typeof p.oneof && ( m = n[p.oneof], "undefined" === typeof m))
											throw Error("Illegal oneof in message " + c.name + "#" + p.name + ": " + p.oneof);
										p = new g.Message.Field(this, c, p.rule, p.type, p.name, p.id, p.options, m);
										m && m.fields.push(p);
										c.addChild(p)
									}
								n = [];
								...
								this.ptr.addChild(c)
							} else if (k.isValidService(a)) {
								c = new g.Service(this, this.ptr, a.name, a.options);
								for (h in a.rpc)a.rpc.hasOwnProperty(h) && c.addChild(new g.Service.RPCMethod(this, c, h, a.rpc[h].request, a.rpc[h].response, a.rpc[h].options));
								this.ptr.addChild(c)
							} else if (k.isValidExtend(a))
			...
			n["import"] = function(b, d) {
				...
				if (b.imports && 0 < b.imports.length) {
					var a,
					    ...
					    g = !1;
					if ("object" === typeof d) {
						if (this.importRoot = d.root,
							g = !0,
							a = this.importRoot,
							d = d.file, 0 <= a.indexOf("\\") || 0 <= d.indexOf("\\"))
							c = "\\"
					} else
						"string" === typeof d ? this.importRoot ? a = this.importRoot : 0 <= d.indexOf("/") ? ( a = d.replace(/\/[^\/]*$/, ""), "" === a && ( a = "/")) : 0 <= d.indexOf("\\") ? ( a = d.replace(/\\[^\\]*$/, ""),
						c = "\\") : a = "." : a = null;
				...
				if (null != this.ptr && "object" !== typeof this.ptr.type) {
					if (this.ptr instanceof g.Namespace) {
						...
						if (d.TYPE.test(this.ptr.type))
							...
							else if ( b instanceof g.Message)
		...
		g.loadProto = function(e, d, l) {
			if ("string" === typeof d || d && "string" === typeof d.file && "string" === typeof d.root)
				l = d,
				d =
				void 0;
			return g.loadJson((new g.DotProto.Parser(e)).parse(), d, l)
		};
		g.protoFromString = g.loadProto;
		g.loadProtoFile = function(e, d, l) {
			d && "object" === typeof d ? ( l = d,
			d = null) : d && "function" === typeof d || ( d = null);
			if (d)
				return g.Util.fetch("string" === typeof e ? e : e.root + "/" + e.file, function(k) {
					if (null === k)
						...
						try {
							d(null, g.loadProto(k, l, e))
						} catch(b) {
							d(b)
						}
				});
			var k = g.Util.fetch("object" === typeof e ? e.root + "/" + e.file : e);
			return null === k ? null : g.loadProto(k, l, e)
		};
			...
			if (d)
				return g.Util.fetch("string" === typeof e ? e : e.root + "/" + e.file, function(k) {
					if (null === k)
						...
						try {
							d(null, g.loadJson(JSON.parse(k), l, e))
						} catch(b) {
							d(b)
						}
				});
			var k = g.Util.fetch("object" === typeof e ? e.root + "/" + e.file : e);
			return null === k ? null : g.loadJson(JSON.parse(k), l, e)
		};
		return g
	}
	"function" === typeof require && "object" === typeof module && module && "object" === typeof exports && exports ? module.exports = u(require("bytebuffer")) : "function" === typeof define && define.amd ? define(["ByteBuffer"], u) : (s.dcodeIO = s.dcodeIO || {}).ProtoBuf = u(s.dcodeIO.ByteBuffer)
})(this);
